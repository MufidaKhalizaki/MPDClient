...
public class StreamFetcher {
    ...
    private static String parse(final String data, final Iterable<String> handlers) {
        final String start = data.substring(0, data.length() < 10 ? data.length() : 10).toLowerCase();
        ...
        } else if ((!data.contains("<html") && data.contains("http:/")) || (data.contains("#EXTM3U"))) { // m3u with comments?
            return parseExt3Mu(data, handlers);
        }
    ...
    public String get(final String url, final String name) {
        String parsed = null;
